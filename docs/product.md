...
- 后端(Linux)负责视频处理（音频提取、字幕烧录），需要 FFmpeg 和充足存储
- 前端上传视频文件到后端，后端处理完成后前端下载导出结果
- 开发阶段：后端可运行在本地 WSL2 或远程 Linux 服务器
